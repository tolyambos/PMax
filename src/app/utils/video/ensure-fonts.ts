...
const FONT_METADATA_PATH = path.join(
  process.cwd(),
  "public",
  "fonts",
  "font-metadata.json"
    ...
    function makeRequest(url: string, redirectCount = 0): void {
      ...
      const req = client.get(url, (response) => {
        ...
        file.on("finish", () => {
          ...
          console.log(
            `[FONT-DOWNLOAD] Downloaded ${fileName} (${(stats.size / 1024).toFixed(1)} KB)`
          );
          resolve();
        });
...
async function ensureFontFile(fileName: string): Promise<string> {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (stats.size > 1000) {
      ...
      console.log(
        `[FONT-ENSURE] ${fileName} is a Git LFS pointer, re-downloading...`
      );
    } else {
      console.log(
        `[FONT-ENSURE] ${fileName} is too small (${stats.size} bytes), re-downloading...`
      );
    }
  }
 ...
 */
export async function ensureFontForRendering(
  fontFamily: string,
  weight: string = "400"
): Promise<string | null> {
  try {
    ...
    const metadata: FontMetadata[] = JSON.parse(
      fs.readFileSync(FONT_METADATA_PATH, "utf8")
    );
    ...
    if (!fontPath && Object.keys(fontMeta.files).length > 0) {
      ...
      console.log(
        `[FONT-ENSURE] Using fallback weight ${fallbackWeight} for ${fontFamily}`
      );
    }
...
export async function preloadAllFonts(): Promise<void> {
  try {
    ...
    for (const font of metadata) {
      for (const fontPath of Object.values(font.files)) {
        const fileName = fontPath.split("/").pop();
        if (fileName) {
          try {
            ...
            console.error(
              `[FONT-ENSURE] Failed to preload ${fileName}:`,
              error
            );
          }
        }
