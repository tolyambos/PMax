...
export default function WizardPage() {
  ...
  const [canCreateProjects, setCanCreateProjects] = useState<boolean | null>(
    null
  );
    ...
    const checkPermissions = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (!data.canCreate) {
            setError(
              data.reason || "You don't have permission to create projects."
            );
          }
        } else {
  ...
  const handleWizardComplete = (projectData: any) => {
    ...
    if (projectData && projectId) {
      console.log("Redirecting to editor:", projectId);
          ...
          <Card className="border-destructive/50 bg-destructive/5">
            ...
            <CardContent className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Please contact an administrator to request project creation
                permissions.
              </p>
